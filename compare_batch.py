...
def compare(vectorstores, question, model=None):
    ...
    for key, vectorstore in vectorstores.items():
        qa_chain = RetrievalQA.from_chain_type(
            llm=llm,
            retriever=vectorstore.as_retriever(search_kwargs={"k": 25}), # 25
...
@hydra.main(version_base="1.3", config_path="conf", config_name="path_to_your_data.yaml")
def main(cfg):
    ...
    counter = 0
    for user_input in cfg.topics:
        save_complete(user_input, vs_dict, cfg.model_name, chunk_size=cfg.chunk_size, overlap=cfg.overlap)
        counter += 1
        logging.info(f"Completed {counter} out of {len(cfg.topics)} comparisons.")
